...
fn test_readonly_class_decorator() {
    ...
    assert!(result.is_ok(), "readonly decorator should compile: {:?}", result.err());
...
fn test_readonly_method_decorator() {
    ...
    let result = compile_and_check(source);
    assert!(result.is_ok(), "readonly method decorator should compile: {:?}", result.err());
    let output = result.unwrap();
    ...
    // Should apply decorator using the plain name (global alias)
    assert!(output.contains("MyClass.getValue = readonly(MyClass.getValue)"),
...
fn test_sealed_class_decorator() {
    ...
    let result = compile_and_check(source);
    assert!(result.is_ok(), "sealed decorator should compile: {:?}", result.err());
    let output = result.unwrap();
    ...
    // Runtime should export global alias
    assert!(output.contains("sealed = TypedLua.sealed"),
            "Runtime should export sealed as global alias");

    // Should apply decorator using the plain name (global alias)
    assert!(output.contains("FinalClass = sealed(FinalClass)"),
...
fn test_sealed_method_decorator() {
    ...
    let result = compile_and_check(source);
    assert!(result.is_ok(), "sealed method decorator should compile: {:?}", result.err());
    let output = result.unwrap();
    ...
    // Should apply decorator using the plain name (global alias)
    assert!(output.contains("MyClass.process = sealed(MyClass.process)"),
...
fn test_deprecated_class_decorator() {
    ...
    let result = compile_and_check(source);
    assert!(result.is_ok(), "deprecated decorator should compile: {:?}", result.err());
    let output = result.unwrap();
    ...
    // Runtime should export global alias
    assert!(output.contains("deprecated = TypedLua.deprecated"),
            "Runtime should export deprecated as global alias");

    // Should apply decorator using the plain name (global alias)
    assert!(output.contains("OldClass = deprecated(OldClass)"),
...
fn test_deprecated_with_message() {
    ...
    let result = compile_and_check(source);
    assert!(result.is_ok(), "deprecated with message should compile: {:?}", result.err());
    let output = result.unwrap();
    ...
    assert!(output.contains("OldClass = deprecated(\"Use NewClass instead\")(OldClass)"),
...
fn test_deprecated_method() {
    ...
    let result = compile_and_check(source);
    assert!(result.is_ok(), "deprecated method should compile: {:?}", result.err());
    let output = result.unwrap();
    ...
    // Should apply decorator using the plain name (global alias)
    assert!(output.contains("MyClass.oldMethod = deprecated(MyClass.oldMethod)"),
...
fn test_deprecated_method_with_message() {
    ...
    let result = compile_and_check(source);
    assert!(result.is_ok(), "deprecated method with message should compile: {:?}", result.err());
    let output = result.unwrap();
    ...
    // Should apply decorator with message using the plain name (global alias)
    assert!(output.contains("MyClass.oldMethod = deprecated(\"Use newMethod instead\")(MyClass.oldMethod)"),
...
fn test_multiple_builtin_decorators() {
    ...
    let result = compile_and_check(source);
    assert!(result.is_ok(), "multiple built-in decorators should compile: {:?}", result.err());
    let output = result.unwrap();

    // Should include runtime library only once
    let runtime_count = output.matches("TypedLua Runtime Library").count();
    assert_eq!(runtime_count, 1, "Runtime library should be included exactly once");
...
fn test_mix_builtin_and_custom_decorators() {
    ...
    let result = compile_and_check(source);
    assert!(result.is_ok(), "mix of built-in and custom decorators should compile: {:?}", result.err());
    let output = result.unwrap();
...
fn test_runtime_library_embedded_only_when_needed() {
    ...
    let result = compile_and_check(source);
    assert!(result.is_ok(), "custom decorator should compile: {:?}", result.err());
    let output = result.unwrap();
...
fn test_builtin_decorator_with_inheritance() {
    ...
    let result = compile_and_check(source);
    assert!(result.is_ok(), "built-in decorator with inheritance should compile: {:?}", result.err());
    let output = result.unwrap();
...
fn test_builtin_decorator_with_interface() {
    ...
    let result = compile_and_check(source);
    assert!(result.is_ok(), "built-in decorator with interface should compile: {:?}", result.err());
    let output = result.unwrap();
...
fn test_builtin_decorator_preserves_class_structure() {
    ...
    let result = compile_and_check(source);
    assert!(result.is_ok(), "built-in decorator should preserve class structure: {:?}", result.err());
    let output = result.unwrap();
...
fn test_all_builtin_decorators_together() {
    ...
    let result = compile_and_check(source);
    assert!(result.is_ok(), "all built-in decorators should compile: {:?}", result.err());
    let output = result.unwrap();
